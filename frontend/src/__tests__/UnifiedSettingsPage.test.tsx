...
describe('UnifiedSettingsPage payment disclosures', () => {
  ...
  it('places processor rates inside the QuickBooks source, not Taxes & Fees', async () => {
    ...
    expect(within(panel).getByRole('region', { name: 'Processing rates' })).toBeVisible()
    const ratesToggle = within(panel).getByRole('button', { name: 'Processing rates' })
    expect(ratesToggle).toHaveAttribute('aria-expanded', 'false')
    expect(within(panel).getByText('2.5%')).not.toBeVisible()
    ratesToggle.focus()
    await user.keyboard('{Enter}')
    expect(ratesToggle).toHaveAttribute('aria-expanded', 'true')
    ...
    await user.click(ratesToggle)
    expect(within(panel).getByText('2.5%')).not.toBeVisible()
    expect(within(panel).getByText('QuickBooks published rates')).toHaveAttribute('tabindex', '-1')
    await user.click(screen.getByRole('button', { name: 'Taxes & Fees' }))
    expect(screen.getByRole('region', { name: 'Processing rates', hidden: true })).not.toBeVisible()
  ...
  it('uses the shared scoped password dialog before saving invoice card routing', async () => {
    ...
    renderPage()
    await user.click(screen.getByRole('button', { name: 'Payments & Accounting' }))
    await user.click(await screen.findByRole('button', { name: 'Change card processor' }))
  ...
  it('puts connected QuickBooks and configured Zelle before unused Stripe in DOM order', async () => {
    ...
    const page = renderPage()
    await user.click(screen.getByRole('button', { name: 'Payments & Accounting' }))
    await screen.findByRole('button', { name: /QuickBooks Online.*CONNECTED/i })
  ...
  it('consolidates one ready processor with its connection and keeps empty credits below methods', async () => {
    const user = userEvent.setup()
    const fallback = apiMocks.get.getMockImplementation()!
    apiMocks.get.mockImplementation((path: string) => {
      if (path === '/payments/settings/card-provider/readiness') return Promise.resolve({ data: { ...DB048_PROVIDER_READINESS, selected_provider: 'quickbooks_payments', stripe_connect: { status: 'not_configured', approved: false, tenant_ready: false }, quickbooks_payments: { status: 'ready', approved: true, tenant_ready: true } } })
      if (path === '/quickbooks/status') return Promise.resolve({ data: { ...quickBooksConnection, is_connected: true, token_health: 'healthy' } })
      if (path === '/stripe/connect/status') return Promise.resolve({ data: { ...stripeConnection, is_connected: false } })
      if (path.includes('credit') && path.includes('aging')) return Promise.resolve({ data: [] })
      return fallback(path)
    })
    renderPage()
    await user.click(screen.getByRole('button', { name: 'Payments & Accounting' }))
    const methods = await screen.findByRole('region', { name: 'Payment methods' })
    expect(within(methods).getByRole('button', { name: /QuickBooks Online/ })).toHaveTextContent('Active for card payments')
    expect(screen.queryByRole('radiogroup')).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Change card processor' })).not.toBeInTheDocument()
    const settings = within(methods).getByRole('button', { name: 'Manage connection' })
    expect(settings).toHaveAttribute('aria-expanded', 'false')
    expect(within(methods).queryByRole('button', { name: 'Disconnect QuickBooks' })).not.toBeInTheDocument()
    await user.click(settings)
    expect(settings).toHaveAttribute('aria-expanded', 'true')
    expect(within(methods).getByRole('button', { name: 'Disconnect QuickBooks' })).toBeVisible()
    expect(apiMocks.post).not.toHaveBeenCalled()
    expect(apiMocks.put).not.toHaveBeenCalled()
    const credits = await screen.findByRole('region', { name: 'Customer credits' })
    expect(methods.compareDocumentPosition(credits) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    expect(credits).toHaveTextContent('No open credits')
    expect(within(credits).queryByRole('button', { name: 'Export CSV' })).not.toBeInTheDocument()
  })
  ...
  it('dismisses the QuickBooks consequence dialog without mutation from every safe exit', async () => {
    ...
    const openConfirmation = async () => {
      const manage = screen.queryByRole('button', { name: 'Manage connection' })
