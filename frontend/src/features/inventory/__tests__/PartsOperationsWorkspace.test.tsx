...
describe('DB-038 Parts Operations workspace', () => {
  ...
  it('requires a reason only when on-hand quantity changes and sends only changed quantity fields', async () => {
    installInventoryControlsFixture()
    const user = userEvent.setup()
    renderGate()
    await user.click(await screen.findByRole('tab', { name: 'Inventory' }))
    ...
    expect(screen.getByText('Required only when the on-hand quantity changes.')).toBeInTheDocument()
  ...
  it('re-homes demand selection and atomically resets its draft form when search changes', async () => {
    ...
    expect(screen.getByLabelText('Order quantity')).toHaveValue(9)
    ...
    expect(screen.getByLabelText('Order quantity')).toHaveValue(3)
