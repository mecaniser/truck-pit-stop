  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <GlassNoirCard><p className="text-xs uppercase tracking-[0.16em] text-zinc-500">Recorded platform fees</p><p className="mt-2 text-3xl font-semibold text-gold-300">{money(ledger.totals.platform_fees)}</p><p className="mt-1 text-sm text-zinc-400">New charges record fee details</p></GlassNoirCard>
      </div>
      ...
      <GlassNoirCard padding="none" className="overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="border-y border-zinc-800 bg-zinc-950/45 text-xs uppercase tracking-[0.12em] text-zinc-500"><tr><th className="px-6 py-3">Date</th><th className="px-4 py-3">Merchant</th><th className="px-4 py-3">Invoice</th><th className="px-4 py-3">Amount</th><th className="px-4 py-3">Platform fee</th><th className="px-4 py-3">Stripe</th></tr></thead><tbody className="divide-y divide-zinc-800">{ledger.entries.map((entry) => <tr key={entry.payment_id}><td className="px-6 py-3 text-xs text-zinc-400">{dateTime(entry.created_at)}</td><td className="px-4 py-3 text-zinc-200">{entry.tenant_name}</td><td className="px-4 py-3 text-zinc-300">{entry.invoice_number}</td><td className="px-4 py-3 font-medium text-zinc-100">{money(entry.amount)}</td><td className="px-4 py-3 text-gold-300">{entry.platform_fee_amount ? `${money(entry.platform_fee_amount)} (${entry.platform_fee_percent}%)` : 'Not recorded'}</td><td className="px-4 py-3"><a href={`https://dashboard.stripe.com/payments/${entry.payment_intent_id}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-sky-300 hover:text-sky-200">Open <ExternalLink className="h-3 w-3" /></a></td></tr>)}</tbody></table></div>
      </GlassNoirCard>
    </div>
  )
}
