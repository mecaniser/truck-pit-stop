...
export default function FullCashPaymentPanel({ invoiceId, summary, onUpdated, onChoosingChange, children }: {
  invoiceId: string
  summary: InvoiceSettlementSummary
  onUpdated: (next: InvoiceSettlementSummary) => void
  onChoosingChange: (choosing: boolean) => void
  children?: (cash: InlineCashTender | undefined) => ReactNode
}) {
  ...
  if (summary.invoice_id !== invoiceId || summary.state === 'paid' || (!allowed && !reason)) return children?.(undefined) ?? null

  const choose = (value: boolean) => {
    if (inFlight.current || (value && !allowed)) return
    setExpanded(value)
  ...
  if (children) return children({ selected: expanded, select: choose, allowed, pending, reason, confirmation })
