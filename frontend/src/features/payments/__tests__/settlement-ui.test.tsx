...
describe('SettlementPaymentPanel', () => {
  ...
  it('shows the server payment hold reason without a misleading disabled tender selector', () => {
    const summary = { ...DB048_SETTLEMENT_FIXTURES.unpaid.summary,
...
describe('EarlyVehicleReleasePanel', () => {
  it('requires an owner/admin reason and preserves the unpaid balance', async () => {
    ...
    expect(screen.getByText(/remains due\./i)).toHaveTextContent(Number(summary.outstanding_balance).toLocaleString('en-US', { style: 'currency', currency: 'USD' }))
    expect(screen.queryByText(/owner\/admin exception|QuickBooks A\/R/i)).not.toBeInTheDocument()
    expect(screen.queryByRole('heading', { name: 'Release vehicle before payment is complete' })).not.toBeInTheDocument()
    expect(screen.getByLabelText(/release reason/i)).toHaveAttribute('placeholder', 'Reason for early release')
    const submit = screen.getByRole('button', { name: 'Release vehicle', exact: true })
    expect(submit).toBeDisabled()
