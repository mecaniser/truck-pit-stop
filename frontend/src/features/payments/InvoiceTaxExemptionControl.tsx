...
// not to a tender. Disclosure -> evidence -> server-confirmed updated total.
export default function InvoiceTaxExemptionControl({ invoiceId, summary, onUpdated, onEditingChange }: {
  ...
  const request = useRef<{ key: string; version: number; reason: string; support_reference: string } | null>(null)
  ...
  const apply = async () => {
    ...
    if (reason.trim().length < 3 || !reference.trim()) return
    request.current ??= { key: createIdempotencyKey(), version: summary.version, reason: reason.trim(), support_reference: reference.trim() }
    ...
    try {
      const next = await applyInvoiceTaxExemption(invoiceId, {
        expected_settlement_version: command.version, reason: command.reason, support_reference: command.support_reference,
      }, command.key)
  ...
  return <section aria-label="Invoice tax exemption" className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-slate-950">
    {exemption.applied ? <div role="status" className="space-y-1">
      ...
      <p className="break-words text-sm text-slate-600">{exemption.reason}</p>
      ...
      {editing && <form id={id} onSubmit={event => { event.preventDefault(); void apply() }} className="mt-2 space-y-3 border-t border-slate-200 pt-3">
        ...
        <label className="block text-sm font-medium" htmlFor={`${id}-reason`}>Exemption reason</label>
        <textarea id={`${id}-reason`} required minLength={3} maxLength={500} rows={2} disabled={pending || request.current !== null} value={reason} onChange={event => setReason(event.target.value)} className="block w-full rounded-lg border border-slate-300 bg-white p-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-700 disabled:opacity-70" />
        <label className="block text-sm font-medium" htmlFor={`${id}-reference`}>Certificate or supporting reference</label>
        <input id={`${id}-reference`} required maxLength={255} disabled={pending || request.current !== null} value={reference} onChange={event => setReference(event.target.value)} className="block min-h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-700 disabled:opacity-70" />
