...
export default function DashboardHome() {
  ...
  // them a few seconds after they appear (staff can still tap through to the
  // linked view before then). Desktop keeps them until manually dismissed.
  ...
  useEffect(() => {
    ...
    // Fade each pill out (staggered) so it reads as a graceful auto-clear.
    const timers = pending.map((k, i) =>
      setTimeout(() => handleDismissAlert(k), 6000 + i * 250)
    )
    ...
    >
      ...
      <div className="flex-shrink-0">
        <div className="flex flex-row items-start justify-between gap-3 lg:items-end">
          ...
          {isManager && (
            <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-2 sm:gap-3 lg:flex-nowrap">
              <button
                onClick={() => setShowQuickForm(!showQuickForm)}
                className="flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 font-semibold text-white transition-colors sm:px-4"
                style={{
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 font-semibold text-white transition-colors hover:bg-white/10 sm:px-4"
