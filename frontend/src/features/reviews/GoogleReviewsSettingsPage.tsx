...
type Location = { account_id: string; location_id: string; name: string; address?: string | null }
...
export default function GoogleReviewsSettingsPage() {
  ...
  const selectLocation = useMutation({ mutationFn: async (location: Location) => api.put('/google-reviews/connection/location', { account_id: location.account_id, location_id: location.location_id, location_name: location.name }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['google-review-connection'] }); queryClient.invalidateQueries({ queryKey: ['google-review-locations'] }); toast.success('Google Business Profile connected') }, onError: () => toast.error('Could not save this location') })
