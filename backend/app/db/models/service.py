...
class Service(BaseModel):
    ...
    # Optional flat-rate override. When NULL, price is computed as
    # tenant.labor_rate * (duration_minutes/60) + Σ(part.selling_price * qty).
...
class ServicePart(BaseModel):
    """Parts attached to a service definition (e.g., 4 batteries for a battery replacement)."""
    __tablename__ = "service_parts"

    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    tenant = relationship("Tenant", backref="service_parts")
