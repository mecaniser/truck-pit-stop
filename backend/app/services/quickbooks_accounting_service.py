...
from app.db.models.inventory_lifecycle import (
    CounterSale, CounterSaleLine, CounterSaleReturn, CounterSaleReturnLine,
)
...
async def ensure_walk_in_customer(
    connection: QuickBooksConnection,
    tenant: Tenant,
) -> str:
    """Resolve the deterministic tenant walk-in customer without creating a CRM customer."""
    if connection.walk_in_customer_id:
        return connection.walk_in_customer_id
    display_name = f"DieselBridge Walk-in · {str(tenant.id)[:8]}"
    matches = await _query(
        connection,
        f"select * from Customer where DisplayName = '{_escape_query(display_name)}' maxresults 1",
    )
    if matches and matches[0].get("Id"):
        connection.walk_in_customer_id = str(matches[0]["Id"])
        return connection.walk_in_customer_id
    response = await _request(
        connection, "POST", "customer",
        json={
            "DisplayName": display_name,
            "CompanyName": (tenant.name or "Walk-in customer")[:255],
            "Notes": f"DieselBridge anonymous counter sales for tenant {tenant.id}",
        },
    )
    customer = response.get("Customer")
    if not isinstance(customer, dict) or not customer.get("Id"):
        raise QuickBooksAccountingError("QuickBooks did not return the walk-in customer")
    connection.walk_in_customer_id = str(customer["Id"])
    return connection.walk_in_customer_id


async def _ensure_counter_sale_item(connection: QuickBooksConnection) -> str:
    item_name = "DieselBridge Counter Sales"
    items = await _query(
        connection,
        f"select * from Item where Name = '{_escape_query(item_name)}' maxresults 1",
    )
    ...
    if not accounts or not accounts[0].get("Id"):
        raise QuickBooksAccountingError("QuickBooks company has no income account for counter sales")
    response = await _request(
        ...
        json={
            ...
            "Type": "Service",
            "IncomeAccountRef": {"value": str(accounts[0]["Id"])},
            "Description": "Parts sold through DieselBridge counter sales",
        },
    )
    item = response.get("Item")
    if not isinstance(item, dict) or not item.get("Id"):
        raise QuickBooksAccountingError("QuickBooks did not return the counter-sale item")
    return str(item["Id"])


async def sync_counter_sale_receipt(
    connection: QuickBooksConnection,
    tenant: Tenant,
    sale: CounterSale,
    ...
    """Create one idempotent QBO Sales Receipt from immutable sale snapshots."""
    if sale.accounting_sales_receipt_id:
        return sale.accounting_sales_receipt_id
    document_number = sale.sale_number[:21]
    matches = await _query(
        connection,
        f"select * from SalesReceipt where DocNumber = '{_escape_query(document_number)}' maxresults 1",
    )
    if matches and matches[0].get("Id"):
        sale.accounting_sales_receipt_id = str(matches[0]["Id"])
    else:
        customer_ref = (
            await ensure_customer(connection, customer)
            if customer is not None
            else await ensure_walk_in_customer(connection, tenant)
        )
        item_id = await _ensure_counter_sale_item(connection)
        provider_lines = []
        provider_total = Decimal("0.00")
        for line in lines:
            allocations = list(line.unit_allocations or [])
            if len(allocations) != int(line.quantity):
                raise QuickBooksAccountingError("Counter-sale unit allocation is incomplete")
            unit_amounts = [
                (
                    Decimal(entry["item"]) + Decimal(entry["tax"])
                    + Decimal(entry["fee"])
                ).quantize(Decimal("0.01"))
                for entry in allocations
            ]
            if sum(unit_amounts, Decimal("0.00")) != Decimal(line.total).quantize(Decimal("0.01")):
                raise QuickBooksAccountingError("Counter-sale unit allocation does not reconcile")
            for index, amount in enumerate(unit_amounts, start=1):
                provider_total += amount
                provider_lines.append({
                    "Amount": float(amount),
                    "Description": (
                        f"{line.sku_snapshot} · {line.name_snapshot}; "
                        f"unit {index}/{line.quantity}"
                    )[:400],
                    "DetailType": "SalesItemLineDetail",
                    "SalesItemLineDetail": {
                        "ItemRef": {"value": item_id},
                        # A one-unit line makes Amount == Qty * UnitPrice exactly,
                        # avoiding provider-side cent drift for indivisible totals.
                        "Qty": 1,
                        "UnitPrice": float(amount),
                    },
                })
        if provider_total != Decimal(sale.total).quantize(Decimal("0.01")):
            raise QuickBooksAccountingError("Counter-sale receipt total does not reconcile")
        response = await _request(
            connection, "POST", "salesreceipt",
            json={
                "DocNumber": document_number,
                "CustomerRef": {"value": customer_ref},
                "TxnDate": (sale.completed_at or datetime.now(timezone.utc)).date().isoformat(),
                "PrivateNote": f"DieselBridge counter sale {sale.sale_number}",
                "Line": provider_lines,
            },
        )
        receipt = response.get("SalesReceipt")
        if not isinstance(receipt, dict) or not receipt.get("Id"):
            raise QuickBooksAccountingError("QuickBooks did not return the counter-sale Sales Receipt")
        sale.accounting_sales_receipt_id = str(receipt["Id"])
    sale.accounting_sync_status = "synced"
    return sale.accounting_sales_receipt_id


async def sync_counter_sale_refund_receipt(
    connection: QuickBooksConnection,
    tenant: Tenant,
    sale: CounterSale,
    return_row: CounterSaleReturn,
    return_lines: list[CounterSaleReturnLine],
    sale_lines: dict[Any, CounterSaleLine],
    customer: Customer | None,
) -> str:
    """Create one idempotent QBO Refund Receipt for a confirmed local refund."""
    if return_row.accounting_refund_receipt_id:
        return return_row.accounting_refund_receipt_id
    document_number = f"R-{str(return_row.id)[:18]}"[:21]
    matches = await _query(
        connection,
        f"select * from RefundReceipt where DocNumber = '{_escape_query(document_number)}' maxresults 1",
    )
    if matches and matches[0].get("Id"):
        return_row.accounting_refund_receipt_id = str(matches[0]["Id"])
        return return_row.accounting_refund_receipt_id
    customer_ref = (
        await ensure_customer(connection, customer)
        if customer is not None
    ...
    for row in return_lines:
        sale_line = sale_lines[row.sale_line_id]
        allocations = {
            int(entry["ordinal"]): entry
            for entry in (sale_line.unit_allocations or [])
        }
        unit_amounts = []
        for ordinal in row.unit_ordinals or []:
            entry = allocations.get(int(ordinal))
            if entry is None:
                raise QuickBooksAccountingError("Counter-sale return allocation is incomplete")
            unit_amounts.append((
                Decimal(entry["item"]) + Decimal(entry["tax"])
                + Decimal(entry["fee"])
            ).quantize(Decimal("0.01")))
        row_total = (
            Decimal(row.item_amount) + Decimal(row.tax_amount) + Decimal(row.fee_amount)
        ).quantize(Decimal("0.01"))
        if len(unit_amounts) != int(row.quantity) or sum(unit_amounts, Decimal("0.00")) != row_total:
            raise QuickBooksAccountingError("Counter-sale return allocation does not reconcile")
        for ordinal, amount in zip(row.unit_ordinals or [], unit_amounts):
            provider_total += amount
            provider_lines.append({
                "Amount": float(amount),
                "Description": (
                    f"Return {sale_line.sku_snapshot} · {sale_line.name_snapshot}; "
                    f"unit {ordinal}; {row.disposition}; {row.reason}"
                )[:400],
                "DetailType": "SalesItemLineDetail",
                "SalesItemLineDetail": {
                    "ItemRef": {"value": item_id},
    ...
    if provider_total != Decimal(return_row.refund_amount).quantize(Decimal("0.01")):
        raise QuickBooksAccountingError("Counter-sale refund receipt total does not reconcile")
    response = await _request(
        connection, "POST", "refundreceipt",
        json={
            "DocNumber": document_number,
            "CustomerRef": {"value": customer_ref},
            "TxnDate": (return_row.completed_at or datetime.now(timezone.utc)).date().isoformat(),
            "PrivateNote": f"DieselBridge counter sale {sale.sale_number} return",
            "Line": provider_lines,
        },
    )
    receipt = response.get("RefundReceipt")
    if not isinstance(receipt, dict) or not receipt.get("Id"):
        raise QuickBooksAccountingError("QuickBooks did not return the counter-sale Refund Receipt")
    return_row.accounting_refund_receipt_id = str(receipt["Id"])
    return return_row.accounting_refund_receipt_id
