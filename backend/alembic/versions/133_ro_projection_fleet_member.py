"""Carry the customer's fleet membership in the repair-order projection.

The repair-order list is served from a materialised JSONB payload built by
refresh_repair_order_read_model, so a field added to the response schema alone
is never populated — it silently reads as its default. The header colour needs
to know whether the carrier is on a fleet plan, which is Customer.fleet_enabled
and is distinct from RepairOrder.is_internal: the shop's own trucks are one
thing, a paying carrier on a fleet plan is another.

Existing rows are reprojected so the value is present for orders that already
exist, not only ones touched after this runs.

Revision ID: 133_ro_projection_fleet_member
Revises: 132_payment_source_step_up
"""

from alembic import op

revision = "133_ro_projection_fleet_member"
down_revision = "132_payment_source_step_up"
branch_labels = None
depends_on = None

NEW_FUNCTION = """CREATE OR REPLACE FUNCTION refresh_repair_order_read_model(target_repair_order_id uuid)
RETURNS void AS $$
        BEGIN
          ...
          customer.fleet_enabled,
            ...
            'customer_fleet_enabled', coalesce(customer.fleet_enabled, false)
            ),
            now()
...
OLD_FUNCTION = """CREATE OR REPLACE FUNCTION refresh_repair_order_read_model(target_repair_order_id uuid)
RETURNS void AS $$
        BEGIN
          DELETE FROM repair_order_read_models WHERE repair_order_id = target_repair_order_id;
...
def _reproject() -> None:
    # Rebuild every stored payload so existing orders carry the new key.
