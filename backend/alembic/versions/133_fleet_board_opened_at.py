"""carry the visit's opened_at on fleet board work orders

A repair order is a visit, so how long it has been open is how long the truck
has been in the shop — the signal that says a visit never closed. The board
reads its cards from this projection, and the payload carried
...
def _refresh_function(include_opened_at: bool) -> str:
    driver_phone_line = "              'driver_phone', vehicle.driver_phone,\n"
    opened_at_line = ",\n              'opened_at', repair_order.created_at" if include_opened_at else ""
    return f"""
...
{driver_phone_line}              'mileage', vehicle.mileage,
          ...
          LEFT JOIN LATERAL (
            SELECT jsonb_build_object(
              ...
              'work_started_at', repair_order.work_started_at{opened_at_line}
            ) AS payload
...
def upgrade() -> None:
    op.execute(_refresh_function(include_opened_at=True))
    op.execute(
        """
...
def downgrade() -> None:
    op.execute(_refresh_function(include_opened_at=False))
    op.execute(
        """
        UPDATE fleet_board_read_models
        SET urgent_work_order = urgent_work_order - 'opened_at'
        WHERE urgent_work_order IS NOT NULL
        """
    )
    op.execute(
        """
        UPDATE fleet_board_read_models
        SET pm_work_order = pm_work_order - 'opened_at'
        WHERE pm_work_order IS NOT NULL
        """
    )
