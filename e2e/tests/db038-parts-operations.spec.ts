...
test('DB-038 contains a bounded master/detail workstation across responsive views without unhandled errors', async ({ browser }, testInfo) => {
  for (const [width, height] of [[1280, 900], [960, 900], [390, 844], [320, 720]]) {
    ...
    await expect(primaryTabs.locator('[role="tab"][aria-selected="true"]')).toHaveCount(1)
    await expect(page.getByRole('region', { name: 'Inventory results, 50 shown of 99 matching', exact: true })).toHaveAttribute('tabindex', '0')
    const inventoryRows = page.locator('[data-parts-row]')
    await expect(inventoryRows).toHaveCount(50)
    await expect(page.getByRole('button', { name: 'Show 49 more parts' })).toBeVisible()
    ...
    for (const label of ['All active', 'Needs reorder', 'Out of stock', 'In stock', 'Archived']) await expect(stockFilters.getByRole('button', { name: label, exact: true })).toHaveCSS('min-height', '44px')
    await stockFilters.getByRole('button', { name: 'Needs reorder', exact: true }).click()
    await expect(page.getByText('2 of 99 active parts')).toBeVisible()
    await stockFilters.getByRole('button', { name: 'Out of stock', exact: true }).click()
    await expect(page.getByText('1 of 99 active parts')).toBeVisible()
    await stockFilters.getByRole('button', { name: 'In stock', exact: true }).click()
    await expect(page.getByText('96 of 99 active parts')).toBeVisible()
    await stockFilters.getByRole('button', { name: 'Archived', exact: true }).click()
    await expect(page.getByText('1 archived part')).toBeVisible()
    await expect(page.getByText(/Archived part/)).toBeVisible()
    await expect(page.getByRole('button', { name: 'Adjust quantities' })).toHaveCount(0)
    await stockFilters.getByRole('button', { name: 'All active', exact: true }).click()
    ...
    await expect(page.getByText('0 of 99 active parts')).toBeVisible()
    ...
    await expect(page.getByText('99 active parts')).toBeVisible()
    await expect(page.getByLabel('Sort inventory')).toHaveValue('name-asc')
    await expectSingleMasterSelection(page, inventoryRows.first())
...
test('DB-038 stock-detail changes preserve item ownership, permissions, validation, and responsive containment', async ({ browser }, testInfo) => {
  for (const [width, height] of [[1280, 900], [960, 900], [390, 844], [320, 720]]) {
    ...
    const selectedHeader = page.locator('.db-parts-operations__selected-part-header')
    ...
    await expect(controls.getByRole('heading', { name: 'Stock details' })).toBeVisible()
    const edit = controls.getByRole('button', { name: 'Adjust quantities' })
    await expect(edit).toBeVisible()
    expect((await edit.boundingBox())!.height).toBeGreaterThanOrEqual(44)
    await edit.click()
    for (const label of ['Available now', 'Incoming quantity', 'Adjustment reason']) {
    ...
    if (width === 1280) {
      await controls.getByLabel('Available now', { exact: true }).fill('8')
      await controls.getByLabel('Incoming quantity', { exact: true }).fill('4')
      await controls.getByRole('button', { name: 'Save quantities' }).click()
      await expect(controls.getByRole('alert')).toHaveText('Adjustment reason is required when available stock changes.')
      expect(updates).toHaveLength(0)
      await controls.getByLabel('Adjustment reason', { exact: true }).fill('Cycle count correction')
      await controls.getByRole('button', { name: 'Save quantities' }).click()
      await expect(page.getByTestId('parts-selection-status')).toContainText('Air filter stock details saved. 8 available.')
      expect(updates).toEqual([{ id: 'inventory-1', body: { stock_quantity: 8, stock_adjustment_reason: 'Cycle count correction', on_order_quantity: 4 } }])
      await expectSingleMasterSelection(page, rows.first())
      await expect(controls).toContainText('8')
      await controls.getByRole('button', { name: 'Adjust quantities' }).click()
      await controls.getByLabel('Incoming quantity', { exact: true }).fill('5')
      await controls.getByRole('button', { name: 'Save quantities' }).click()
      ...
      await controls.getByLabel('Reorder point', { exact: true }).fill('6')
      await controls.getByRole('button', { name: 'Save reorder point' }).click()
      expect(updates[2]).toEqual({ id: 'inventory-1', body: { reorder_level: 6 } })
      await controls.getByRole('button', { name: 'Update average cost' }).click()
      await controls.getByLabel('Average unit cost', { exact: true }).fill('12.75')
      await controls.getByRole('button', { name: 'Save average cost' }).click()
      expect(updates[3]).toEqual({ id: 'inventory-1', body: { cost: 12.75 } })
      await expect(controls).toContainText('$12.75')

      await controls.getByRole('button', { name: 'Adjust quantities' }).click()
      await controls.getByLabel('Available now', { exact: true }).fill('77')
      ...
      await nextControls.getByRole('button', { name: 'Adjust quantities' }).click()
      await expect(nextControls.getByLabel('Available now', { exact: true })).toHaveValue('2')
  ...
  await failureControls.getByLabel('Reorder point', { exact: true }).fill('9')
  await failureControls.getByRole('button', { name: 'Save reorder point' }).click()
  await expect(failureControls.getByRole('alert')).toHaveText('Inventory update could not be saved.')
  await expect(failureControls.getByLabel('Reorder point', { exact: true })).toHaveValue('9')
  ...
  await expect(readOnlyControls).toContainText('You can view stock details. Owners and admins can make changes.')
  await expect(readOnlyControls.getByRole('button', { name: 'Adjust quantities' })).toHaveCount(0)
