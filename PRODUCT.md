...
- **Driver identity, truck custody, and portal access are separate records and actions.** Decided 2026-08-10. A durable `DriverProfile` exists independently of a login and accumulates custody, inspection, incident, and accountability history. Assigning or replacing a truck's driver starts a new custody period and closes the prior one without rewriting history. Driver Portal access is granted only through an explicit, tenant-scoped WorkOS invitation linked to the selected profile; creating a profile or assigning a truck never grants login access implicitly.
