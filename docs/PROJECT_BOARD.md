...
| DB-002 | P0 | Review | Ship the supported paid repair-order conversion export | Backend & Integrations | Branch `codex/conversion-export` is rebuilt on current `main`. Migration 108 remains single; additive migration 117 stores PII-minimized sensitive-action audits. Security remediation pins delivery to a maximum four-address vetted DNS snapshot while retaining TLS hostname verification, bounds DNS and total delivery time, restricts controls to owner/`conversion_exports`, serializes authoritative corrections, signs `timestamp.body`, adds versioned key rotation, enforces an absolute created-at PII expiry across every outbox state, and validates HTTP(S) landing URLs. Evidence: latest security-focused exact-pinned suite 42 passed/1 isolated-PostgreSQL skip; isolated PostgreSQL full-chain upgrade, 117 downgrade/upgrade, and simultaneous multi-key correction test previously passed; exact-pinned full backend suite is 627 passed/4 skipped | Third fresh independent Architecture/Security re-review and QA gate; configure the production versioned keyring and private-egress policy; prove worker/beat, signed receiver, audit and retention runtime; focused PR/CI, Railway deploy and canary |
