...
| DB-032 | P2 | Review — letter drop verified | Animate the public DieselBridge wordmark on first page load | Frontend & UX | Product review rejected the bridge-traversal treatments. The isolated branch now keeps the bridge static and drops all 12 wordmark letters for 560ms from deterministic varied heights, offsets, rotations, and delays before they settle into the unchanged final lockup; reduced motion uses a 120ms no-travel fade. Focused landing tests passed `5/5`; full frontend passed `173/173`; production build and changed-file lint/diff passed; homepage Playwright passed `4/4` with a sampled multi-height drop frame, reduced-motion behavior, and zero overflow at 390/320. No API, auth, tenant, data, backend, dependency, worker, or migration change. | Product assigns fresh independent QA for motion feel, first-load behavior, reduced motion, and desktop/mobile layout before release. The implementing Frontend owner must not self-approve. |
