...
| DB-032 | P2 | Review — refined motion verified | Animate the public DieselBridge wordmark as a vehicle crossing its bridge mark on first page load | Frontend & UX | Isolated branch `codex/db032-logo-motion` replaces the rejected rigid jump with a 12-letter wave: each letter originates over the stationary bridge, travels independently for 620ms with a 28ms stagger, and settles without layout shift; reduced motion uses a 120ms no-travel fade. Focused landing tests passed `5/5`; full frontend passed `173/173`; production build and changed-file lint/diff passed; homepage Playwright passed `4/4` with a sampled multi-position crossing frame, reduced-motion behavior, and zero overflow at 390/320. No API, auth, tenant, data, backend, dependency, worker, or migration change. | Product assigns fresh independent QA for motion feel, first-load behavior, reduced motion, and desktop/mobile layout before release. The implementing Frontend owner must not self-approve. |
