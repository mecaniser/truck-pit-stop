...
| DB-035 | P1 | Combined QA/Security remediation complete — fresh independent gates pending | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Frontend & UX Recovery Owner | Independent QA returned candidate `2bc9043fe8c87e5175b0e8b16fb7302dfffe3eab` because its committed Playwright bootstrap was not deterministic in a clean clone; independent Security returned the same candidate because `GET /dashboard/daily-workset` lacked staff-role/object authorization. The bounded combined remediation now gives Playwright an isolated strict-port server, deterministic fixture/router state, exact new/legacy hierarchy assertions, and fail-closed request/console handling, while the backend permits only owner/admin/receptionist/mechanic and rejects denied or inactive contexts before protected projection SQL. Exact DB-035 Playwright passes `6/6` twice in fresh processes; focused frontend passes `9/9`; full frontend passes `257/257`; production build, direct changed-file lint, diff integrity, and Impeccable detector pass. Backend focused authorization/DB-035/WorkOS/WebSocket passes `121/121`; the correctly configured full backend passes `757` with `4` skips; a clean archive reports sole migration head `118_authenticated_presentation`. No API shape or migration was added. | Fresh independent Security and QA must gate the new exact integrated SHA. No DB-036, push, PR, merge, deployment, release, or implementation-owner self-gate is authorized. |
| DB-035A | P0 | Backend Security P1 remediated and integrated — fresh gates pending | Replace the rejected transition-only Shop Work → Repair Orders topology with one persistent canonical repair-order workbench: Shop Work is today's tenant-local navigator; Repair Orders is all-record discovery. | Backend & Integrations | Independent Security returned candidate `2bc9043fe8c87e5175b0e8b16fb7302dfffe3eab`: authenticated customer/driver principals could read tenant-wide `GET /dashboard/daily-workset`. Backend remediation `cbaac25d9e75d96471b1e6624e2d2868c03ba14c` was integrated once as `9874f5429567e1bd6ef2415beb4915869b8abe1d`; it permits only garage_owner, garage_admin, receptionist, and mechanic and denies before protected projection SQL. Combined verification passes the focused `121/121` set and the correctly configured full backend `757 passed, 4 skipped`; a clean candidate archive reports sole head `118_authenticated_presentation`. No API shape or migration was added. | Fresh independent Security and QA gate the exact integrated candidate; do not release on implementation-owner evidence alone. |
