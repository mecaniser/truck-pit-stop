...
| DB-031 | P1 | Review — independent gates pending | Make exception error persistence deterministic | Backend & Integrations | Dedicated branch candidate from base `a973eb3` now freezes one bounded request-free persistence envelope before constructing response background work; all error writes use a dedicated owned session. One lifecycle owns semaphore acquisition, session operations, phase-aware cleanup, and permit release through a single hard deadline. Unknown commits invalidate exactly once; confirmed commits close with bounded invalidate fallback; precommit failures rollback/close with bounded invalidate fallback; separate phase cancellation waves cover commit, refresh, rollback, close, invalidate, close→invalidate, and rollback→invalidate without overlapping session operations. Exact acquire-cancel, external cancellation with twice-resistant commit, late permit reclaim, request/exception/user GC and background closure scan, TestClient label, secret absence, Stripe dedicated-session behavior, response parity, and concurrency coverage pass. Pinned Python 3.11 evidence: lifecycle `28/28` plus three fresh-process repetitions `28/28` each; combined lifecycle/security/schema/request-performance/QuickBooks `117/117`; original QuickBooks regression five fresh processes `1/1` each; complete backend `736 passed, 4 skipped`; compile and diff checks pass. No migration, dependency, push, PR, merge, deploy, release, Security GO, or QA GO is claimed. | Product assigns fresh independent Security and QA reviewers to the final clean local SHA. Security must re-check the frozen-envelope retention boundary, dedicated-session ownership, cancellation/unknown-commit cleanup, and secret-safe fallback logging. QA must rerun the pinned focused/full suites and original QuickBooks ordering regression. Release remains pending both GO verdicts, protected CI, deploy health, canary, observation, and rollback evidence; Backend may not self-approve. |
