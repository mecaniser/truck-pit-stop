...
| DB-037 | P1 | Ready for bounded implementation — Architecture GO | Provide one MVP local controller that switches fixed ports 5173/8000 between clean registered worktrees without mixed frontend/backend sources and shows `branch@shortSHA` in development. | Release & Reliability | MVP contract v1 is recorded in `docs/DB-037_LOCAL_RUNTIME_CONTROLLER_CONTRACT.md` on `codex/db037-local-runtime-controller`, based on `origin/main` `4cbaec940c8ed6bb7b7bdf04950f5a5eb3d11c89` after DB-035 PR #264 merged. V1 uses the existing configured local PostgreSQL/Redis, refuses every dirty target, requires exact process/container ownership and migration compatibility, performs one verified rollback attempt, persists secret-free local state, and injects two non-secret Vite development identity values. Per-worktree data isolation, dirty authorization, daemon supervision, signed metadata endpoints, worker control, and extended rollback are explicitly deferred. No runtime, application, data, dependency, migration, push, PR, merge, or deployment change is included. | Release delivers one small controller commit and freezes the identity fixture; Frontend then delivers one small indicator commit. Fresh independent Security and QA gates are required before Product may authorize a push/PR/merge. |
