...
| DB-015 | P0 | In progress | Remediate WebSocket token logging | Security & Identity | Architecture approved a backend-first, cookie-only `/ws` contract. Security first returned NO-GO at `681d37d`; the identity/correlation remediation at `96ba6b6` received combined code Security GO, but runtime QA then returned NO-GO because a WorkOS DRIVER reached the tenant-wide staff channel. Commit `84a82c4` restricted registration to owner, garage-admin, receptionist, and mechanic, after which fresh Security returned NO-GO because WebSocket authorization still trusted a potentially stale local `User.role` instead of the validated WorkOS membership role. The current remediation requires an explicitly mapped `TenantMembership.role_slug` to match `User.role` exactly; unknown or divergent roles fail before registration and on every live revalidation. Synchronized provider downgrades to driver/fleet also remain outside the staff channel. Customers still require an exact active tenant/customer link and register only on their own channel. Focused WebSocket plus WorkOS is 101/101, combined auth/identity authority is 147/147, protected-critical plus DB-015 is 120/120, and full backend is 714 passed / 4 skipped; compile, diff check, and the single Alembic head 117 are green. No migration, ticket store, or new secret is required. | Return this remediation to fresh independent Security and runtime QA. Require both GO, authenticated event-isolation and live-role-change capture, focused PR/CI, backend-first deployment, and canary before closure. |
