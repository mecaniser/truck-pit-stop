...
| DB-031 | P1 | Review — fresh independent gates pending | Make exception error persistence deterministic | Backend & Integrations | Independent QA and Security returned candidate `cca1b89a9fba090fad8be2e34d789f56b5f3f5e5` for two P1s: one of three fresh Python 3.11 lifecycle runs delivered only one of two required resistant commit-cancellation waves, and arbitrary nested `request_context` objects could survive redaction without JSON/size bounds. Backend remediation replaces wall-clock wave polling with event-loop acknowledgement barriers plus lifecycle phase-change futures; the strict two-wave assertion remains unchanged and production persistence contains no sleep/grace masking. Context is now synchronously detached into only dict/list/string/finite number/bool/null with depth, total-item, key, string, and serialized-byte ceilings; sensitive keys/values are redacted, unsupported/cyclic/deep/large/hostile graphs fail entirely closed, caller-owned inputs are released before lifecycle construction, and safe Stripe scalar context remains intact. Pinned Python 3.11 evidence: exact read-only lifecycle/context command `32/32`, then twelve fresh stress containers `32/32` each; original QuickBooks regression five fresh containers `1/1` each; combined lifecycle/security/schema/request-performance/QuickBooks `121/121`; complete backend with the repository WorkOS issuer `740 passed, 4 skipped`. The first full command used the lifecycle-gate WorkOS issuer and produced one unrelated signature-test failure; that test passed `1/1` after correcting the harness, followed by the green complete run. Compile/diff pass; sole Alembic head remains `117_conversion_export_security`. No migration, dependency, push, PR, merge, deploy, release, Architecture approval, Security GO, or QA GO is claimed. | Product assigns fresh Architecture, Security, and QA reviewers to the final clean local SHA. Architecture rechecks phase/barrier ownership and hard-deadline behavior; Security rechecks primitive-only context detachment, bounds, redaction, GC, and Stripe compatibility; QA repeats the exact fresh-process lifecycle stress, QuickBooks x5, combined focused, and complete pinned backend suite. Backend may not self-approve. |
