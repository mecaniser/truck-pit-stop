...
| DB-035A | P0 | Implementation complete — independent gates pending | Replace the rejected transition-only Shop Work → Repair Orders topology with one persistent canonical repair-order workbench: Shop Work is today's tenant-local navigator; Repair Orders is all-record discovery. | Frontend & UX Recovery Owner | Architecture GO is recorded in `docs/DB-035_SHARED_WORKBENCH_CONTRACT_V2.md`. The additive read-only `GET /dashboard/daily-workset` projection is present and consumed by the shared workbench; `GET /dashboard/action-queue` remains compatible. Product's one-at-a-time inline brief remains derived from already-loaded navigator fields: **Details** does not request data or change selected ID/URL/history, while record activation and **Open repair order** select the canonical workspace. Deterministic projection/workbench tests and the authenticated responsive matrix are green; routes, permissions, mutations, statuses, legacy/new rollback, and Repair Orders ownership remain intact. | Fresh independent Security and QA on the exact candidate; no additional frontend-owned projection work or new backend handoff is required. |
