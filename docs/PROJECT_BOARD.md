...
| DB-038 | P1 | Current-main integration implemented — fresh Security and QA pending | Turn DieselBridge Inventory into a tenant-safe parts-operations workspace that connects repair demand and replenishment to purchasing, receiving, stock history, cores, and vendor returns. | Backend & Integrations | Architecture Contract v1 remains recorded in `docs/DB-038_PARTS_OPERATIONS_CONTRACT.md`. Historical independent Security GO remains attached to exact backend/read-contract candidate `92aed013522c936ddb1c8bfce0cddd1993d665a3`; historical QA and Release verification remain attached to the prior gated candidate `4b59921ca1b9a8ff28a7c7dd196729c7871d784f`. Current-main application integration is `772418d7dcf6641dfb65b511c88e5d821beb121d` on `codex/db038-main-reconcile`, based on `a1b7be30cdf314ca2dd0df232f92470c30523596`; it preserves ETS retirement/import behavior, DB-038 Parts Operations, and linearizes the migration as sole revision `124_parts_operations_v1` after main revision 123. Implementation-owner verification in the available sandbox passed backend generic/read `7/7`, Fleet/ETS compatibility `55/55`, focused frontend `5/5`, full frontend `298/298`, production build, regression-aware changed lint, compile, OpenAPI preflight (`381` paths; `19` DB-038 paths), sole-head declaration, and diff integrity. The complete Python 3.12 backend ran `850 passed, 37 skipped, 2 failed`; both failures reproduce unchanged on parent main (`a1b7be3`): loopback bind is denied by sandbox policy and the separate current-month report fixture returns an empty row set at the UTC/tenant-local boundary. Docker/PG15, exact Playwright, and pinned Python 3.11 remain required external gates because this sandbox denies Docker and listener access and cannot download the 3.11 dependencies. Attachments remain deferred, rollout gates remain off, and no production migration or tenant enablement occurred. | Fresh independent Security and QA must gate the final immutable integrated candidate, including tenant/role non-enumeration, durable idempotency, ETS compatibility, migration 0→124 and 124→123→124, focused/full frontend, strict DB-038 Playwright, and proportional/full backend evidence before any PR or release action. |
