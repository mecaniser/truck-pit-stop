...
| DB-032 | P2 | Review — QA NO-GO remediated | Animate the public DieselBridge wordmark on first page load | Frontend & UX | Independent QA rejected `a45dda9` because visible letters crossed the viewport/header ceiling for 217–252ms and collided in flight. The remediation keeps every letter in its reserved horizontal slot, limits deterministic vertical travel to 5–9px inside the measured header-safe region, shortens the drop to 480ms, and preserves the 120ms reduced-motion fade. Focused landing tests passed `5/5`; full frontend passed `173/173`; build, changed-source lint, and diff-check passed; homepage Playwright passed `7/7`, including complete 0–680ms transformed-box sampling with zero header escapes/collisions at 1366/390/320; a separate video configuration passed `3/3` natural cached-reload journeys with current videos and early frames. Bridge/footer remained unanimated and no layout overflow was introduced. No API, auth, tenant, data, backend, dependency, worker, or migration change. | Product assigns a fresh independent QA reviewer to inspect the new natural-load frames/videos and re-run the failed clipping/overlap gate. The implementing Frontend owner must not self-approve. |
