...
| DB-035 | P1 | Released — tenant rollout and reload fix verified | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Frontend & UX Recovery Owner | PR #264 introduced the authenticated presentation; the Truck Pit Stop tenant-wide default remains `new` for 8 active users with zero active per-user overrides and global force-legacy unset. PR #268 application candidate `b99cdb355529a74a4c99b1ce6bce6f7321aa46fa` received independent QA GO and Security GO with `P0/P1/P2=0`; focused Theme/Appearance/Auth tests passed `28/28`, full frontend passed `262/262`, all six protected checks and the production build passed. PR #268 squash-merged as `4970ac50864d24b3ffc11a3f45f8e498a3f0d9ca`; reviewed application/test blobs are identical. Railway production deployment `be0ad414-bb9e-45eb-95c0-6450a3af9ffc` is `SUCCESS`. An eligible authenticated Truck Pit Stop staff session showed `data-presentation=new` before and after a full refresh, browser error logs were empty, and web/API health returned HTTP 200. | Closed. Keep the tenant default `legacy` as the single rollback control. Truck Pit Stop Wisconsin remains legacy; 77 Cargo remains fleet-company context; no user override, environment variable, API/schema/data migration, route, permission, or business state changed. |
