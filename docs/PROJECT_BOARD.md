...
|---|---|---|---|---|---|---|
| DB-003 | P1 | In Progress — reconcile with released DB-035 | Finish versioned additional-work authorizations without regressing the authenticated garage cockpit | Backend & Integrations | Authoritative DB-003 candidate `2451f740ccb2e9bb5c0d5b94e56f2bde0ec6a5fe` is being rebased onto current `origin/main` `79ee5d39eaeb67f6631308b5f58700ebfcd3a9b6` in isolated branch `codex/db003-db035-reconciliation`. DB-035 workstation structure and tests are preserved while DB-003 authorization behavior is integrated. The competing DB-003 migration is renumbered to `119_ro_projection_cardinality` after released `118_authenticated_presentation`; no production migration or release action has occurred. | Complete focused migration/backend/frontend/Playwright verification on the reconciled exact SHA, then assign fresh independent Security and QA. Only after dual GO and protected CI may Product authorize merge/deploy. |
