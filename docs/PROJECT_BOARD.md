...
| DB-035 | P1 | Product recovery — selected prototype correction | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Product & Delivery Lead | Accepted baseline `a7b7fc0879af2b811ccf5c04b224133e6f7e9d0c` retains the backend/Appearance foundation and bounded Harden cleanup. Product rejected Adapt `5c8c698` because a rail and queue rearrangement did not deliver the connected operational outcome despite green automation. The controlled plan is `docs/DB-035_MASTER_EXECUTION_PLAN.md`. The canonical local pair is one recovery checkout on frontend `5173` with strict-port failure and backend `8000`; historical DB-035 ports are not fallbacks. Product selected Action Ledger plus a compact Record Atlas connected-record strip as topology, but rejected the prototype's all-dark admin styling as inconsistent with the public ProductWorkspace. | Recovery owner revises only the isolated Action Ledger prototype into the landing-derived navy-shell/road-white-workspace product language at 1440/960/390/320. Product compares it directly with the public tour before any production implementation. Fresh independent Security, QA and Impeccable/Emil gates follow the complete candidate. |
