...
| DB-035 | P1 | Stage 4 Product review | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Frontend & UX Recovery Owner | Product accepted Stage 3 at `ab79e7e56076d26c0b6231ccc7730042df2020a9`. The Stage 4 candidate keeps the legacy Repair Orders page as immediate rollback and recomposes only the new presentation into a compact canonical order ledger plus selected repair-order context. Focused component/parity tests pass `24/24`; TypeScript/build, changed lint and diff integrity pass. Fixture-routed 1440/960/390/320 browser evidence proves exact ID/lane deep links, return context, no horizontal overflow, 44px coarse-pointer controls, reduced motion and forced colors with zero console errors. Product rail review removed the redundant full-rail icon in favor of the shared animated landing wordmark, retains only DB mark + tenant logo + state in the compact rail, and anchors the named garage-owner profile in a dedicated bottom account area; focused shell/landing tests pass `8/8` and the bounded responsive pass has no page overflow. A bounded Payments & Accounting Impeccable follow-up keeps payment behavior intact while making 320/390 accordion headers readable, associating disclosure triggers with labelled regions, replacing legacy 20px card elevation with a contained two-layer elevation shared by Settings and the Appearance preview, and resolving Stripe record values through Day Shop foreground tokens; focused settings tests, changed lint/build/diff and live 320/390/960/1440 checks pass. The canonical `5173`/`8000` service pair now runs from the recovery worktree. The backend targets the prepared `truckpitstop_db035` recovery database: tenant/user ID sets and the repaired projection function match the populated database, Alembic reports `118_authenticated_presentation (head)`, and the original database remains unchanged. Health is 200 and the WorkOS login endpoint redirects normally. | Product completes a real WorkOS login and visual/interaction review. Stage 5, independent QA/Security and release remain unauthorized. |
| DB-035A | P0 | In progress — shared daily workbench | Replace the rejected transition-only Shop Work → Repair Orders topology with one persistent canonical repair-order workbench: Shop Work is today's tenant-local navigator; Repair Orders is all-record discovery. | Frontend & UX Recovery Owner | Architecture GO is recorded in `docs/DB-035_SHARED_WORKBENCH_CONTRACT_V2.md`: `Tenant.timezone` is the IANA local-day source, and both Stripe/manual payment finalization atomically write `Invoice.paid_at` with the paid repair-order state. The compatible path is a new read-only `GET /dashboard/daily-workset`; `GET /dashboard/action-queue` remains byte-compatible. Product selected a one-at-a-time inline order brief derived only from already-loaded navigator fields. **Details** never requests data or changes selected ID/URL/history; the record body and the brief's secondary **Open repair order** control are the two explicit ways to select that canonical record. Legacy/new flag, routes/query keys, ownership, permissions, mutations, statuses, Sidekick behavior, and prior DB-035 evidence remain intact. | Implement the additive daily projection and extract the shared canonical workspace behind the existing flag. Then require focused tests, deterministic local-midnight fixtures, bounded visual/interaction evidence, and independent QA/Security. No PR, merge, or deployment is authorized. |
| DB-035B | P2 | In progress — utility inbox IA | Make the new-presentation authenticated rail describe work, management, and communication rather than one undifferentiated destination list. | Frontend & UX Recovery Owner | Product selected the Utility Inbox direction in `docs/DB-035_UTILITY_INBOX_NAVIGATION_DECISION.md`: Shop Work → Repair Orders → Customers stays contiguous; My Shop is a visually distinct management entry; Messages moves to the account-adjacent utility while retaining the existing role gate and unread affordance. Routes, permissions, unread query, legacy presentation, and mobile More navigation remain authoritative. | Complete focused shell tests, production build, changed-file lint, and bounded authenticated desktop/compact/mobile review. Independent QA remains required before release; no backend/API/migration, PR, merge, or deployment is authorized. |
| DB-035C | P0 | Navigator correction checkpoint — browser acceptance pending | Replace the new-presentation customer Sidekick primary detail with a canonical page-owned customer workstation while preserving legacy rollback. | Frontend & UX Recovery Owner | Product accepted `docs/DB-035_CUSTOMER_WORKSTATION_CONTRACT.md`. The implementation uses additive, query-preserving `?selected={customerId}` state, a wide navigator + persistent workspace, compact list-to-workspace transition, explicit Back/focus restoration, in-workspace vehicle drilldown, existing queries/mutations/permissions only, and unchanged legacy Sidekick. The navigator correction replaces the compressed table with flat responsive records containing customer/company name, primary phone, **Details**, and explicit **Open customer** activation. A single full-width inspected brief reuses already-loaded/cached account, contact, vehicle, relationship, and history facts; inspection can differ from selection and cannot navigate, mutate `?selected`, replace the workspace, or start a parallel request. Focused workstation/inspection/width regressions pass `12/12`; direct changed-file ESLint reports only the same 19 pre-existing `no-explicit-any` violations in `CustomersPage.tsx`; this delta adds none. No backend/API/migration/business-logic/auth change was made. | Complete production build/diff integrity, then the bounded authenticated 1440/960/390/320 overflow, selected-versus-inspected, themes/forced-color/computed-contrast review and final Impeccable detector. The current in-app session is redirected to `/login?reason=workos_session_expired`; sign-in is required in that same tab for runtime evidence. Preserve the excluded recovery wiring and conflicting untracked migration. No PR, merge, or deployment is authorized. |
