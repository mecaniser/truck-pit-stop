...
| DB-031 | P1 | Review — fresh independent gates pending | Make exception error persistence deterministic | Backend & Integrations | Independent QA returned clean candidate `de04b4e03d3ed7103fd1094c5340b1bbbe8f0f3e` after two fresh `32/32` lifecycle/context passes when the third process exposed a P1 cohort-admission race: five timed-out attempts could construct a third session after a failed hanging-close owner released one of two permits. Backend remediation captures the permit generation before waiting; a failed or cancelled lifecycle advances that generation only after its exact close/invalidate settles and immediately before releasing its permit, so pre-existing waiters cannot replace failed cohort work while later healthy requests remain admissible. The strict `len(sessions) == 2` assertion remains unchanged, and a new event-driven five-lifecycle barrier covers cleanup settlement, stale waiters, exact permit reclamation, and next-generation admission without sleeps. Prior fixes also remain intact: deterministic two-wave phase acknowledgement and deeply bounded primitive-only JSON-safe context detachment/redaction. Pinned Python 3.11 evidence: lifecycle/context `33/33`, followed by twenty fresh stress processes at `33/33` each (`660/660` aggregate); original QuickBooks regression five fresh processes at `1/1` each; combined lifecycle/WebSocket security/QuickBooks/schema/request-performance `122/122`; two complete backend runs each `741 passed, 4 skipped`, with no pending-task, `GeneratorExit`, or `IllegalStateChangeError`; standalone schema preflight `6/6`; compile and diff checks pass; sole Alembic head remains `117_conversion_export_security`. No migration, dependency, push, PR, merge, deploy, release, Architecture approval, Security GO, or QA GO is claimed. | Product assigns fresh independent Architecture, Security, and QA reviewers to the new clean local commit. Architecture rechecks generation fencing, exact permit ownership, cleanup settlement, and hard deadlines; Security rechecks primitive-only context retention/redaction and dedicated-session behavior; QA repeats the exact fresh-process stress, QuickBooks x5, combined focused, and complete pinned backend gates. Backend may not self-approve. |
