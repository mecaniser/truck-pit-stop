...
| DB-037 | P1 | Ready to merge — clean local cutover, Security GO, QA GO, CI green | Provide one MVP local controller that switches fixed ports 5173/8000 between clean registered worktrees without mixed frontend/backend sources and shows `branch@shortSHA` in development. | Release & Reliability | Exact application candidate `613e9a97638ae7565cba81586de15a303bb605fa` closes database selection/status redaction and runtime-identity exposure, while Playwright uses an isolated identity-free test mode. Fresh independent Security GO and QA GO report `P0/P1/P2=0`; controller `39/39`, frontend critical `81/81`, full frontend `259/259`, production build, Python compilation, lint, diff integrity, secret/identity canaries, and all six protected PR #265 checks pass. The controller-owned runtime is healthy on fixed ports `5173/8000` from the same clean worktree and SHA with `truckpitstop_db035` at sole Alembic head/current `118_authenticated_presentation`; PostgreSQL/Redis and data were preserved. | Merge focused PR #265 after final board-evidence CI, then verify main ancestry. DB-037 remains local-only tooling; production authenticated-presentation rollout/activation is a separate DB-035 release action. |
