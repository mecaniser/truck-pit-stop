...
| DB-038 | P1 | Frontend candidate implemented — independent QA pending | Turn DieselBridge Inventory into a tenant-safe parts-operations workspace that connects repair demand and replenishment to purchasing, receiving, stock history, cores, and vendor returns. | Frontend & UX | Architecture Contract v1 is recorded in `docs/DB-038_PARTS_OPERATIONS_CONTRACT.md`. Independent Security GO is recorded for exact backend/read-contract candidate `92aed013522c936ddb1c8bfce0cddd1993d665a3` (parent `c4b42c294528fecf98b38b94355b786e428cdc69`) with P0/P1/P2 `0/0/0`. The frontend candidate keeps the existing Inventory catalog as the 403/404 feature-off fallback; a non-gate server failure is explicit and does not simulate enablement. It consumes the frozen normalized fixture through the contract endpoints, provides real repair-order deep links and owner/admin-only idempotent operational actions, and retains read-only reception behavior. Attachments remain deferred and unreachable for Phase 1. | Fresh independent QA must verify the exact integrated candidate at desktop, iPad, 390, and 320, including feature-off fallback, role gates, keyboard tabs, strict console/network behavior, and legacy Inventory compatibility. No frontend candidate may enable a tenant flag, alter backend/API/migrations, or reach production before that gate. |
