...
| DB-038 | P1 | Ready for Backend — Architecture GO | Turn DieselBridge Inventory into a tenant-safe parts-operations workspace that connects repair demand and replenishment to purchasing, receiving, stock history, cores, and vendor returns. | Backend & Integrations | Architecture Contract v1 is recorded in `docs/DB-038_PARTS_OPERATIONS_CONTRACT.md`: existing Inventory remains canonical; immutable transaction-coupled movements extend current repair reservation protections; PO partial receiving uses durable tenant-scoped idempotency and moving WAC; vendor stock returns preserve WAC; cores use a separate custody bucket; tenant non-enumeration, normalized category/supplier compatibility, migration `122_parts_operations_v1`, dual rollout gates, rollback, deterministic fixtures, and responsive UI acceptance are explicit. Scope remains limited to activity+demand, purchasing+receiving, and returns/cores; counter sales, parts-only invoices/payments, AI extraction, vendor integrations, production EasyTruck writes, and broad reporting remain excluded. No unresolved purchasing/costing fork remains. | Backend & Integrations implements the smallest flag-off foundation PR from the accepted contract, including migration/models, shared movement service, API schemas, PostgreSQL concurrency/idempotency/tenant tests, and repair-order/legacy compatibility. Then independent Security gates the exact Backend SHA before Frontend & UX implements the gated workspace; independent QA/Impeccable gates integration. No production migration, enablement, push, merge, or deploy is authorized by this handoff. |
