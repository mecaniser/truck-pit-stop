...
| DB-031 | P1 | Ready to Release | Make exception error persistence deterministic | Backend & Integrations | Exact clean candidate `212a9bfcb0c855323ef95f9efb2a799a1a62eaf5` preserves the strict `len(sessions) == 2` guard and adds generation-fenced permit ownership, deterministic two-wave cleanup acknowledgement, and bounded primitive-only JSON-safe context detachment/redaction. Architecture GO reported no P0–P2. Constitutional fallback Security GO reported no P0–P2 after `264/264` simultaneous resilience plus secret checks; pressure misses were classified as test-clock starvation with ownership correct. Dedicated QA passed twenty fresh pinned Python 3.11 lifecycle/context processes (`660/660`), QuickBooks five fresh processes (`5/5`), combined focused `122/122`, and two complete backend runs at `741 passed, 4 skipped` each with clean teardown scans. Release fallback QA independently verified the exact SHA/scope, preserved assertion, Python 3.11 read-only compile, diff integrity, sole Alembic head `117_conversion_export_security`, and identical pre/post tree. No migration, dependency, frontend, API-contract, auth, tenant, or payment behavior change. | Release & Reliability ships one focused protected PR; require all six CI contexts, successful predeploy schema checks, exact-SHA production/performance web and worker deployments, health plus sanitized error-persistence/QuickBooks/Stripe canaries, clean lifecycle logs and metrics, rollback readiness, and live evidence before Done. |
