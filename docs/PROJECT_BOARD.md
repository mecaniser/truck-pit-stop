...
| DB-035 | P1 | Ready for implementation — Architecture GO | Translate the source-grounded landing design language into the authenticated staff app with controlled old/new presentation rollout | Backend & Integrations, then Frontend & UX | Architecture Contract v1 is recorded in `docs/DB-035_AUTHENTICATED_PRESENTATION_CONTRACT.md`. It fixes one-router/domain-state parity, server-resolved global → user → tenant → legacy rollout precedence, instant scoped rollback, a dedicated tenant/user appearance-preference table with additive migration, versioned bootstrap/read/apply/reset APIs, optimistic revisions, exactly-once migration of the four inventoried localStorage keys, immutable semantic-token boundaries, identity-bound cache/hydration, reversible preview, shared fixtures, negative cases, and the complete appearance/viewport/accessibility/parity matrix. Architecture GO is contract approval only; no UI, API, schema, migration, auth, business-logic, production, push, PR, merge, or deploy change exists yet. | Backend & Integrations implements the additive migration, shared legacy/WorkOS bootstrap, preference and rollout APIs, tenant isolation, concurrency, and focused tests. Frontend & UX then implements the shared presentation/token shell and six surfaces against v1 fixtures. Independent Security, QA, and Impeccable/Emil gates are required before release. |
