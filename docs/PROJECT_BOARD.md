...
| DB-038 | P1 | Backend read-contract correction implemented — awaiting Release verification | Turn DieselBridge Inventory into a tenant-safe parts-operations workspace that connects repair demand and replenishment to purchasing, receiving, stock history, cores, and vendor returns. | Backend & Integrations | Architecture Contract v1 is recorded in `docs/DB-038_PARTS_OPERATIONS_CONTRACT.md`. Security GO is recorded for exact backend candidate `c4b42c294528fecf98b38b94355b786e428cdc69` (parent `cd97358`): generic `5/5` and fresh PostgreSQL 15 `19/19`, with P0/P1/P2 `0/0/0`. Attachments remain explicitly deferred and unreachable for Phase 1. Frontend preflight found the committed read API does not yet match the approved UI contract: `GET /demand` emits empty `sources` and has no state/supplier/search/pagination inputs; purchase-order, return, core, and activity lists likewise omit contracted filters/pagination. | Architecture & API Contracts confirmed the committed read schemas violated contract §4/§11 while mutation, idempotency, role, and tenant gates remain accepted. Backend corrected only the GET projections: compatible list-or-envelope pagination, contracted filters, stable UUID-tied ordering, tenant-scoped filter IDs, supplier/entity summaries, real editable repair shortages, remaining submitted/partial PO supply, return/core source linkage, and frozen shared fixture serialization. Focused generic and PostgreSQL-gated read/security tests now cover exact fixture output, totals, filtering, source quantities, terminal/locked/deleted exclusion, non-enumeration, roles, and feature-off fallback. Release must execute the focused generic and fresh PostgreSQL read matrices, then return the immutable candidate to fresh independent Security before Frontend begins. No frontend application code, flag behavior, tenant setting, migration, provider, remote, or production state was changed. |
