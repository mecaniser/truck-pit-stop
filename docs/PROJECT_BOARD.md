...
| DB-035 | P2 | Gate Review — Repair Orders controls aligned | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Frontend & UX | The tenant rollout and reload fix remain released. Focused branch `codex/db035-repair-order-control-alignment` now gives search and status controls one 44px height token, removes only the Repair Orders search wrapper's extra inset, and gives the selected-record header action/status column one 184px width token plus the existing 3px focus-ring inset. Mobile retains its full-width create action and compact filter grid. Repair Orders/Dashboard focused tests pass `49/49`; production frontend build and diff integrity pass. Presentation-only CSS/test scope; no API, data, auth, tenant, permission, mutation, dependency, or migration impact. | Independent QA verifies equal heights/baseline/right edge, focus treatment, 44px targets, and desktop/mobile reflow on the exact candidate. Then push PR, require protected CI, merge/deploy, and browser-measure the production geometry. |
