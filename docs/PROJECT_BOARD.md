...
| DB-035 | P1 | Ready to Merge — reload hydration gates passed | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Frontend & UX Recovery Owner | PR #264 merged as `4cbaec940c8ed6bb7b7bdf04950f5a5eb3d11c89`; the Truck Pit Stop tenant-wide default remains `new` for 8 active users with zero active per-user overrides and global force-legacy unset. Production reproduced new presentation reverting to legacy after refresh. PR #268 application candidate `b99cdb355529a74a4c99b1ce6bce6f7321aa46fa` makes the identity-scoped `/auth/me/appearance` read immediate while retaining cancellation, cache/error fallback, role eligibility, and unsaved appearance drafts. Independent QA GO and Security GO returned `P0/P1/P2=0`; focused Theme/Appearance/Auth tests pass `28/28`, full frontend passes `262/262`, all six protected checks, production build, and diff integrity pass; changed lint has zero errors and the same eight pre-existing Fast Refresh export warnings. | Merge/deploy PR #268 after the documentation-only gate record re-passes protected checks. Release then uses an eligible authenticated Truck Pit Stop staff identity for a full refresh/new-session canary: `/auth/me/appearance` succeeds, the shell remains `new`, no repeated-fetch/client/API errors appear, and an excluded fleet identity makes no staff appearance request. Rollback remains the tenant default `legacy`. |
