...
| DB-035 | P1 | Product recovery — prototype selection | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Product & Delivery Lead | Accepted baseline `a7b7fc0879af2b811ccf5c04b224133e6f7e9d0c` retains the backend/Appearance foundation and bounded Harden cleanup. Product rejected Adapt `5c8c698` because a rail and queue rearrangement did not deliver the connected operational outcome despite green automation. The controlled plan is `docs/DB-035_MASTER_EXECUTION_PLAN.md`. The canonical local pair is one recovery checkout on frontend `5173` with strict-port failure and backend `8000`; historical DB-035 ports are not fallbacks. Isolated Action Ledger and Record Atlas prototypes have browser and Impeccable evidence but no production code authority. | Product selects Action Ledger plus a compact connected-record strip (recommended) or Record Atlas. Only then may the replacement implementation owner begin the staged shared-shell → Shop Cockpit → Repair Orders plan. Fresh independent Security, QA and Impeccable/Emil gates follow the complete candidate. |
