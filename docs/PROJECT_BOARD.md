...
| DB-035 | P1 | Gate Review — reload hydration fix ready | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Frontend & UX Recovery Owner | PR #264 merged as `4cbaec940c8ed6bb7b7bdf04950f5a5eb3d11c89`; the Truck Pit Stop tenant-wide default remains `new` for 8 active users with zero active per-user overrides and global force-legacy unset. Production reproduced new presentation reverting to legacy after refresh. The focused `codex/db035-presentation-reload` fix makes the identity-scoped `/auth/me/appearance` read immediate while retaining cancellation, cache/error fallback, role eligibility, and unsaved appearance drafts. Focused Theme/Appearance/Auth tests pass `28/28`; production frontend build and diff integrity pass; changed lint has zero errors and the same eight pre-existing Fast Refresh export warnings. | Independent Security and QA review the exact candidate before merge/deploy. Acceptance: stale legacy bootstrap resolves to new immediately after reload, an in-progress appearance preview is not overwritten, excluded fleet identities do not call the staff endpoint, malformed/error responses remain safe, and an authenticated Truck Pit Stop refresh canary remains new. |
