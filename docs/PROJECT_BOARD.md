...
| DB-002 | P0 | Review | Ship the supported paid repair-order conversion export | Backend & Integrations | Branch `codex/conversion-export` is rebuilt on current `main`. Migration 108 remains single; additive migration 117 stores PII-minimized sensitive-action audits. Security remediation pins delivery to one vetted DNS snapshot while retaining TLS hostname verification, restricts controls to owner/`conversion_exports`, serializes authoritative corrections, signs `timestamp.body`, adds versioned key rotation, and enforces PII retention/erasure. Evidence: 56 focused tests pass locally; isolated PostgreSQL full-chain upgrade, 117 downgrade/upgrade, and simultaneous multi-key correction test pass; exact pinned dependency full backend suite is 620 passed/4 skipped | Fresh independent Architecture/Security re-review and QA gate; configure the production versioned keyring and private-egress policy; prove worker/beat, signed receiver, audit and retention runtime; focused PR/CI, Railway deploy and canary |
