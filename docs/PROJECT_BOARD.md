...
| DB-032 | P2 | Review — bridge traversal verified | Animate the public DieselBridge wordmark as a vehicle crossing its bridge mark on first page load | Frontend & UX | Isolated branch `codex/db032-logo-motion` keeps the bridge and wordmark as distinct layers: the complete label travels horizontally over the stationary bridge for 960ms while its 12 letters form a 420ms reverse-staggered wave, then settles to the bridge's right without overlap; reduced motion uses a 120ms no-travel fade. Focused landing tests passed `5/5`; full frontend passed `173/173`; production build and changed-file lint/diff passed; homepage Playwright passed `4/4` with a sampled bridge-traversal frame, reduced-motion behavior, and zero overflow at 390/320. No API, auth, tenant, data, backend, dependency, worker, or migration change. | Product assigns fresh independent QA for motion feel, first-load behavior, reduced motion, and desktop/mobile layout before release. The implementing Frontend owner must not self-approve. |
...
- On the public landing page's first render, only the header `DieselBridge`
  wordmark enters from the left. Each letter floats over the stationary bridge
  in a sequenced wave and settles at its existing final position without layout
  shift.
