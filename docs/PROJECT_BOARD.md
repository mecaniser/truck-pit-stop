...
| DB-035C | P0 | Implementation checkpoint — browser acceptance pending | Replace the new-presentation customer Sidekick primary detail with a canonical page-owned customer workstation while preserving legacy rollback. | Frontend & UX Recovery Owner | Product accepted `docs/DB-035_CUSTOMER_WORKSTATION_CONTRACT.md`. The implementation now uses additive, query-preserving `?selected={customerId}` state, a wide navigator + persistent workspace, compact list-to-workspace transition, explicit Back/focus restoration, in-workspace vehicle drilldown, existing queries/mutations/permissions only, and unchanged legacy Sidekick. Focused workstation tests pass `4/4`, the full frontend passes `243/243`, TypeScript and the production build pass, diff integrity passes, and the final Impeccable detector reports no mechanical findings. No backend/API/migration/business-logic/auth change was made. | Reacquire an authenticated controllable browser and complete 1440/960/390/320 selection/deep-link/Back/vehicle review plus appearance/forced-color/computed-contrast acceptance. Direct changed-file ESLint remains blocked by 19 pre-existing `no-explicit-any` violations in `CustomersPage.tsx`; the DB-035C diff adds none. No PR, merge, or deployment is authorized. |
