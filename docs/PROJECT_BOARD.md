...
|---|---|---|---|---|---|---|
| DB-028 | P1 | Review | Restore the approved rich repair-order product proof on the public homepage | Frontend & UX | Product identified a material fidelity gap after DB-027: the approved mockup's credible repair-order numbers, vehicle/meter/financial details, orange routed connectors/nodes, and fully external approval/invoice/payment evidence cards were simplified into generic placeholders and short neutral rules. Product Page Kit classified the lost density as missing mechanism proof, not decoration. Implemented on `codex/homepage-rich-workspace`: the preview is visibly labeled illustrative and carries one coherent fictional repair-order record; three desktop glass evidence cards remain outside the product frame and use routed signal-orange connectors/nodes; stage changes update the matching proof; compact layouts retain all evidence in a stacked reading order. First QA NO-GO on undersized desktop Sign in and inconsistent tax was remediated with a global 44px target and a reconciled $4,210.42 subtotal + $284.20 tax = $4,494.62 estimate/invoice/payment chain. First craft NO-GO on first-fold completion, glass edge, suppressed active lift, responsive documentation, and active-card edge overlap was remediated: the entire desktop scene ends by 991px at 1366×1000, cards have a faint edge, active scale/translate executes, the 1280/960 contract matches code, and transform-aware geometry keeps active cards external. Fresh independent QA and Impeccable re-reviews are GO with no P0–P2 findings. Final local evidence: landing unit 5/5, complete frontend suite 121/121, production build, exact-file ESLint, desktop/mobile Playwright 2/2 across 1440/1366/1280/1120/960/390/320, post-fix 1366/390 visual recaptures, and `git diff --check` pass. No real PII, testimonial, performance, pricing, API, data, auth, tenant, or migration change. | Create the focused PR, require all CI, merge, verify Railway SHA and production desktop/mobile acceptance, then record release evidence and move Done. |
