...
|---|---|---|---|---|---|---|
| DB-003 | P1 | Release hold — production healthy, browser harness correction QA GO | Finish versioned additional-work authorizations without regressing the authenticated garage cockpit | Backend & Integrations | PR #272 squash-merged the reconciled DB-003/DB-035 stack as `a600eb1314630a51be8420f9cee043bc1406ac28` after independent Security and QA GO and all six protected contexts passed. The linear `118_authenticated_presentation` → `119_ro_projection_cardinality` migration completed successfully. Railway production web `38583e84-ec0f-4284-80c4-8a9654325434`, performance web `3fe87a9d-af60-4f77-a75e-194a8425a043`, and worker `169e18ad-130e-4342-be75-9708832d51e1` are `SUCCESS` on the merge SHA; homepage, API health, and readiness returned HTTP 200 during `2026-08-23T07:00:47Z–07:00:48Z`, and deployment/worker scans were clean. Release stayed held because primary QA proved the committed DB-003 Playwright config lacked the runtime identity required by current Vite. Focused candidate `de589a858ab2d5cd8895abd9c33df0789e99ff07` corrects only `e2e/playwright.db003.config.ts`: deterministic validated branch/full-SHA identity, exact two specs/11 tests, strict port, and no existing-server reuse. With ambient identity unset, the unchanged committed config passed `11/11` in `18.6s`; exact-file lint, production build, identity-leak scan, diff hygiene, and clean-worktree checks passed. Fresh fallback QA GO on `de589a8` recorded `P0/P1/P2=0/0/0`. No observed production authorization or health failure required rollback. | Push one focused test-harness correction PR, require protected CI, merge it under the existing Product authorization, then record the correction merge SHA and close the DB-003 release hold. Authenticated production journey evidence remains limited to available authorized sessions and must not be fabricated. |
