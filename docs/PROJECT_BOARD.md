...
| DB-035C | P0 | Implementation checkpoint — browser acceptance pending | Replace the new-presentation customer Sidekick primary detail with a canonical page-owned customer workstation while preserving legacy rollback. | Frontend & UX Recovery Owner | Product accepted `docs/DB-035_CUSTOMER_WORKSTATION_CONTRACT.md`. The implementation uses additive, query-preserving `?selected={customerId}` state, a wide navigator + persistent workspace, compact list-to-workspace transition, explicit Back/focus restoration, in-workspace vehicle drilldown, existing queries/mutations/permissions only, and unchanged legacy Sidekick. The additive inspection contract is now implemented: every new-presentation record has a flat, single-open **Details** brief derived only from row facts and already-cached canonical contact/vehicle/history queries; inspection can differ from selection and cannot navigate, mutate `?selected`, replace the workspace, or start a parallel request. Explicit record activation or **Open customer** alone selects. Focused workstation/inspection tests pass `7/7`; full frontend passes `246/246`; production build and diff integrity pass; the final Impeccable detector reports no findings. Direct changed-file ESLint reports only the same 19 pre-existing `no-explicit-any` violations in `CustomersPage.tsx`; this delta adds none. No backend/API/migration/business-logic/auth change was made. | Complete the bounded authenticated 1440/960/390/320 selected-versus-inspected review and themes/forced-color/computed-contrast matrix. The isolated Playwright profile cannot reuse Product authentication and is redirected after `401` from `/auth/workos/me`. Preserve the excluded recovery wiring and conflicting untracked migration. No PR, merge, or deployment is authorized. |
