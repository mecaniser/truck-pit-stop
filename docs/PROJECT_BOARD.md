...
| DB-031 | P1 | Review — independent gates pending | Make exception error persistence deterministic | Backend & Integrations | Local implementation `e2f367862b42d58485f24495147a7f8253dfa96d` replaces orphaned fire-and-forget exception-log tasks with response-scoped Starlette background work that completes before request teardown. Scope is exactly `backend/app/main.py` and `backend/tests/test_database_resilience.py`; HTTP, database, Stripe, and global exception responses retain their existing status, headers, sanitized body, correlation, authorization, and tenant behavior. Focused database-resilience/schema/request-performance/QuickBooks coverage passed `22/22`; the exact missing-QuickBooks-credentials `503` regression passed `1/1`; backend compile and diff checks passed. No frontend, dependency, schema migration, worker-task, push, PR, merge, deploy, release, or independent approval is claimed. | Product assigns fresh independent Security and QA reviewers to the final clean local SHA. Release remains pending both GO verdicts, protected CI, deploy health, canary, observation, and rollback evidence; Backend may not self-approve. |
