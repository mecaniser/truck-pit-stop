...
| DB-038 | P1 | Explicit selection recovery complete; fresh independent QA pending | Turn DieselBridge Inventory into a tenant-safe parts-operations workspace that connects repair demand and replenishment to purchasing, receiving, stock history, cores, and vendor returns. | Frontend & UX | Architecture Contract v1 remains recorded in `docs/DB-038_PARTS_OPERATIONS_CONTRACT.md`. PR `#281` merged at `87990dc38ee22acf860936450fce912e0b488900`, but all three Railway builds failed before predeploy because production TypeScript included `PartsOperationsWorkspace.test.tsx`, whose repository-only fixture import is unavailable in the frontend Docker copy. No migration, container cutover, tenant enablement, or production DB-038 canary occurred; web and worker remained healthy on rollback target `7782c594501ca19b44cd5c919dbba8ac9fb58f63`. Focused remediation candidate `892e832dfcd3aa89249bbc4a90c5bd29b8f0cf4a` has current-main parent `87990dc38ee22acf860936450fce912e0b488900` and changes only `frontend/tsconfig.json` to exclude test files from production TypeScript. The current UX recovery fixes the reproduced 100-row containment failure with bounded, independent list/detail scroll regions, loaded-data search/triage, one primary tablist, and concise per-panel context for purchasing/receipts, ledger activity, and return/core custody. The P0 demand-selection mismatch is fixed: a search/filter change clears selection and draft values, collection reads retain page metadata and aggregate all advertised pages, panel/detail failures remain explicit with retry, mutations are single-flight with durable per-attempt idempotency keys, and list/tab focus is roved. QA found one deterministic harness-only P1 on exact `310eaa74`: selected-tab assertions searched descendants rather than the actual tab elements. The focused correction scopes `[role="tab"][aria-selected="true"]` to each tablist. Product then reopened visual acceptance on exact `e716b64c` for a reproduced global-style collision that painted a road-white inner search input inside the dark composed search shell. The focused Impeccable recovery makes the Demand and Inventory wrappers the sole visible search controls, owns focus with `:focus-within`, and gives standalone PO and receipt inputs active-surface foreground/background tokens across light, dark, and high-contrast modes. Computed browser assertions verify transparent borderless search inputs, readable text/placeholder/caret, wrapper focus, and compliant standalone fields. The clean-start DB-038 Playwright gate passes `2/2` across 1280/960/390/320 with desktop/mobile and PO/receipt captures; focused components pass `11/11`; production build, focused lint, Impeccable detector, and diff integrity pass. On parent `77db81e133a0d1316becf861acecf23be374b495`, the focused inventory-operate correction restores canonical fixed-size part thumbnails in Demand and Inventory with neutral missing/broken-image fallback; adds mutually exclusive All/Needs reorder/Out/In stock filters, stable catalog/stock/name sorting, supplier/location search, result/reset context, selection clearing, stock-state announcements, and preserved roving row focus. Focused component tests pass `13/13`; TypeScript, exact-file ESLint, changed-file lint, production build, Impeccable detector (`[]`), and diff integrity pass. The deterministic Playwright fixture discovers `2` tests but its isolated Vite server cannot bind in this sandbox (`listen EPERM 127.0.0.1:5181`); bounded retries failed before test execution. Existing localhost `5173` showed an unauthenticated login runtime and could not be proven to serve this exact candidate, so no committed Playwright or visual-capture success is claimed and independent QA must run the 1280/960/390/320 browser gate. Independent QA returned exact candidate `9f8faf87fafc7d86b85306e135501ff8b52827d4` NO-GO (`P0/P1/P2=0/2/0`) because the browser spec used a fuzzy unscoped `Needs reorder` locator and its strict console collector treated the intentional broken-thumbnail 404 as an unexpected error. The focused correction scopes exact filter controls to the named Inventory stock-filter group, proves the exact broken-thumbnail URL returns 404 before asserting its neutral fallback, and exempts only that correlated URL/status resource error while preserving all other console/page/request failures. The fresh browser run also exposed and fixed a real compact-layout defect: the sticky mobile list toolbar intercepted pointer activation of the visible reset action, so the toolbar is now non-sticky below 760px. The exact DB-038 Playwright gate passes `2/2` in `11.5s` across 1280/960/390/320; focused components pass `13/13`; TypeScript, exact/changed lint, production build, and diff integrity pass. No backend, migration, tenant, data, permission, or flag changed; attachments remain deferred. On parent `f7ca197c70b417eccca7a4d0497681121df07fa9`, the focused catalog-imagery correction uses the active tenant branding projection and implements a bounded fallback chain for Demand and Inventory: canonical part photo, then tenant/company logo placeholder, then the neutral package icon. Selected items now expose a fixed-ratio 144–176px desktop / 96–120px compact image near the detail header with truthful photo/logo/icon semantics and no layout shift. Focused components pass `14/14`; TypeScript, exact-file lint, production build, Impeccable detector (`[]`), and diff integrity pass. The isolated DB-038 Playwright server remains sandbox-blocked before execution (`listen EPERM 127.0.0.1:5181`); its `2` tests are discovered, and supplemental 1280/960/390/320 inspection from the exact worktree mount showed contained image geometry and no runtime errors, but the visible development identity was stale, so no exact-candidate browser pass is claimed. On parent `2401d98645e87a4aa60b14d50a450f1836046470`, the focused selection correction explicitly selects and exposes the first visible Demand, Inventory, Purchase Order, Return, or Core record when its master surface activates; preserves a still-visible selection across search/filter/sort; re-homes to the first remaining row or clears both row/detail when empty; makes Returns/Cores activation deterministic; and keeps exactly one selected row and one roving tab stop. Automatic Demand selection also resets PO draft quantity/cost atomically to the selected inventory record. Focused components pass `16/16`; exact frontend ESLint, TypeScript, production build, Impeccable detector (`[]`), Playwright discovery (`2` tests), and diff integrity pass. The isolated Playwright runtime remains sandbox-blocked before execution (`listen EPERM 127.0.0.1:5181`), so no new browser screenshots or exact-candidate runtime success are claimed. | Fresh independent QA must run the isolated DB-038 1280/960/390/320 browser gate and re-gate the exact explicit-selection candidate before a PR. Do not merge, deploy, migrate production, or enable any tenant flag. |
