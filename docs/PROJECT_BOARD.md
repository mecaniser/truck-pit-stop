...
| DB-038 | P1 | Security NO-GO remediated — awaiting Release verification | Turn DieselBridge Inventory into a tenant-safe parts-operations workspace that connects repair demand and replenishment to purchasing, receiving, stock history, cores, and vendor returns. | Backend & Integrations | Architecture Contract v1 is recorded in `docs/DB-038_PARTS_OPERATIONS_CONTRACT.md`. Backend implementation started from architecture SHA `1d669e2`: additive migration `122_parts_operations_v1`, flag-off tenant/config gates, immutable movement service wired into legacy receipts and repair reservations/releases, normalized supplier/category compatibility, and the purchasing/receipt/return/core API foundation are under focused implementation. Release & Reliability found P1 migration failures during disposable PostgreSQL validation: PostgreSQL does not support `min(uuid)` in the legacy preferred-supplier backfill, and the first downgrade attempted to drop `inventory_categories` before `inventory.category_id`'s FK. Backend replaced the supplier selection with a windowed exact-match (`supplier_count = 1`) and reordered downgrade operations to remove inventory FK columns before the referenced category table; focused regression guards cover both. Migration cycles now pass in Release's disposable PostgreSQL gate. A third focused API P1 found Redis's generic replay cache preempting DB-038's durable replay header; DB-038 routes now bypass that ephemeral cache so only transaction-coupled durable replays emit `Idempotency-Replayed: true`. The non-enumeration regression now compares status/detail while independently requiring distinct safe correlation IDs. Release must rerun focused API then remaining matrices. Backend added PostgreSQL-only executable receipt coverage for partial/final receipt WAC, over-receipt atomicity, durable replay, multiline rollback, and row-lock SQL; further contract matrix coverage remains in progress and is not yet a gate pass. Architecture Amendment 1 defers PO attachments from Phase 1 because the repository has only hosted-image Cloudinary uploads and generated invoice PDFs, not a reusable private document-storage/download boundary. Attachments do not participate in quantities, WAC, stock movements, approvals, demand, or return origins; no attachment route, fixture, UI, Security, or QA acceptance is required. The draft metadata table may remain empty/unreachable or be removed before Backend commit. No production migration, tenant enablement, remote mutation, or frontend work occurred. | Backend candidate verified locally by Release: DB-038 generic `5/5`, behavioral PostgreSQL `16/16`, compatibility `65/65`, PG price/repair races `11/11`, former fleet regression `1/1`, full backend `850 passed, 17 skipped`, migration cycles/backfill green, teardown scan clean, and diff check passed. Attachments remain deferred/unreachable. Fresh independent Security returned NO-GO on `cd973583dba9d8afca6faf5b827ddf31699922da`: PO submit/cancel, return ship/reverse, and core/return state transitions accepted a key without persisting a durable replay record. Backend routed each affected Phase 1 transition through the common tenant/family/principal/route/payload-fingerprint lifecycle; matching retries return the stored status/body with `Idempotency-Replayed: true` and mismatches return 409. The PostgreSQL suite now covers serial replay and separate-session same-key races for PO submit/cancel, core recover/waive, and return submit/ship/credit/cancel/reverse, with replay state snapshots. Release must run this focused matrix and fresh independent Security must gate the resulting local commit before Frontend begins. |
