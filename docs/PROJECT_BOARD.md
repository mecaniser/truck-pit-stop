...
| DB-015 | P0 | Ready to release | Remediate WebSocket token logging | Security & Identity | Architecture approved the backend-first cookie-only `/ws` contract. Independent Security CODE GO and runtime QA GO cover 112 focused backend tests, 714 passed / 4 skipped full backend tests, 8 allowed roles, 8 denied roles, 4 invalid customer contexts, legacy and token-null WorkOS sessions, old-client ignored-query compatibility, live role/customer/tenant/logout closure, and no stale or cross-tenant events. Secret, JWT, Bearer, cookie, ErrorLog, and Redis scans found zero raw credentials. Build, lint, diff, and Alembic `0→117` are green; no migration, ticket store, or new secret is required. | Ship the backend-focused PR first and require all protected CI contexts. Canary both web services and the worker with the old deployed client, cookie-only legacy/WorkOS sessions, live authority changes, and a synthetic marker proven absent from edge, proxy, application, observability, ErrorLog, and Redis evidence. Release the queryless frontend only after backend canary GO. |
