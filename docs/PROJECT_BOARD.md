...
| DB-035 | P1 | Shape correction — source-grounded wireframes required | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Frontend & UX | Architecture preserves the implemented flag/preference/API/migration/Appearance foundation and adds the source-grounded UX/IA correction in `docs/DB-035_AUTHENTICATED_PRESENTATION_CONTRACT.md` §§14–18. Structural change is authorized, but every region must map to existing routes, entities, fields, actions, permissions and statuses. The current token/cache/Appearance, identity hierarchy and presentation hooks are compatible foundation; a CSS/rail reskin is insufficient, and the invented Dashboard `OperationalWorkstation`/local selected-order model is rejected. Backend foundation remains `f314ab9`; dirty Frontend files are unapproved and untouched by Architecture. | Frontend & UX returns source-annotated before→after wireframes and interaction models for Dashboard/Shop Cockpit, Repair Orders, Customers, Messages, My Shop and Settings at desktop/iPad/mobile, plus a per-file retain/revise/remove ledger. Product must explicitly approve Shape before Harden → Adapt → Optimize → Polish code work resumes; Security, QA and Impeccable/Emil gates follow implementation. |
