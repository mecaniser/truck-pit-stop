...
| DB-037 | P1 | Ready for verified local cutover — fresh Security and QA GO | Provide one MVP local controller that switches fixed ports 5173/8000 between clean registered worktrees without mixed frontend/backend sources and shows `branch@shortSHA` in development. | Release & Reliability | Superseding exact application candidate `f9836674725a2cc914af5c3b4657f84c86bd1b9f` safely selects one validated non-secret configured local database name, binds Compose and schema preflight to that exact database, verifies the live API container database, rechecks schema/readiness in terminal-restart status, and closes PostgreSQL/Redis/generic URI credential redaction. Fresh independent Security GO and QA GO report `P0/P1/P2=0`; controller `38/38`, Python compilation, diff integrity, secret canaries, actual `truckpitstop_db035` revision `118_authenticated_presentation`, and mutation-free real mixed-port refusal pass. No listener, container, data, migration, production, push, PR, merge, or deployment mutation occurred during the gates. | Release may stop only the already inspected unmanaged API container, run the clean dry-run, and perform one controller-owned local switch. Preserve PostgreSQL/Redis and record exact runtime health. DB-037 remains local-only and is not deployed to production. |
