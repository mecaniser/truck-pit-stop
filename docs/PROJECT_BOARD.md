...
| DB-038 | P1 | Discovery — product scope accepted | Turn DieselBridge Inventory into a tenant-safe parts-operations workspace that connects repair demand and replenishment to purchasing, receiving, stock history, cores, and vendor returns. | Architecture & API Contracts | Product accepted a focused first release after a read-only audit of the authenticated EasyTruck Inventory, Categories, Out of stock, Orders, Returns, Vendors, Sales, Activity, part-detail, pricing-matrix, and reporting surfaces. In scope: immutable stock-movement history; unified repair/reorder demand queue; normalized categories and suppliers; purchase-order lifecycle with partial receiving/backorders, attachments and idempotent receipts; weighted-average-cost handling; vendor/core return lifecycle; tenant/role boundaries; responsive DieselBridge workstation UI. Explicitly out of scope: direct counter parts sales, customer invoices/payments for parts-only sales, AI document extraction, vendor integrations, production EasyTruck writes, and broad reporting beyond the operational controls needed for this release. Acceptance requires no negative stock, no cross-tenant existence/data disclosure, deterministic concurrent receiving, traceability from demand through receipt/return, preserved repair-order parts behavior, accessible desktop/iPad/390/320 journeys, reduced-motion/forced-colors coverage, focused backend/frontend tests, migration verification, independent Security and QA, and no deployment without separate authority. | Architecture records the versioned data/API contract, authorization matrix, migration/compatibility strategy, concurrency/idempotency rules, rollback flag, shared deterministic fixtures, and a smallest-slice backend/frontend handoff. Return to Product only if two materially different purchasing or costing behaviors remain plausible. |
