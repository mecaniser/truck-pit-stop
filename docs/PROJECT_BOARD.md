...
| DB-035 | P1 | Implementation complete — independent gates pending | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Frontend & UX Recovery Owner | The finite implementation candidate now includes the shared daily workbench, utility inbox, reversible committed Appearance lifecycle/Fleet return context, page-owned Repair Orders workspace, page-owned Customers workstation, canonical shared search treatment, and protected selected-versus-inspected interactions. Full frontend passes `257/257`; focused Customers/Repair Orders pass `37/37`; focused workbench/shell/Appearance pass `53/53`; production build, direct changed-file lint, diff integrity, and final Impeccable detector pass. Authenticated browser acceptance covers Shop Work, Repair Orders, Customers, Messages, My Shop, and Settings at `1440/960/390/320`, with no page overflow, 44px compact Messages controls, theme/contrast checks, and Appearance draft/apply rollback verification. The canonical `5173`/`8000` runtime is sourced from the recovery worktree. | Fresh independent Security and QA must gate the exact local candidate. No DB-036, push, PR, merge, deployment, release, or implementation-owner self-gate is authorized. |
| DB-035A | P0 | Backend Security P1 remediated — fresh gates pending | Replace the rejected transition-only Shop Work → Repair Orders topology with one persistent canonical repair-order workbench: Shop Work is today's tenant-local navigator; Repair Orders is all-record discovery. | Backend & Integrations | Independent Security returned candidate `2bc9043fe8c87e5175b0e8b16fb7302dfffe3eab`: authenticated customer/driver principals could read tenant-wide `GET /dashboard/daily-workset`. The bounded backend remediation now permits only garage_owner, garage_admin, receptionist, and mechanic, and denies before the tenant/action-queue/order projection; deleted staff and missing/inactive/deleted tenant contexts fail closed. Focused daily-workset/DB-035/WorkOS/WebSocket set passes `121/121`; isolated PostgreSQL 15 reaches sole head `118_authenticated_presentation`; a PostgreSQL denial probe records zero route SQL. The one pinned full suite stopped on pre-existing WorkOS issuer fixture mismatch (`test_provider_access_token_signature_and_authoritative_claims` with `WORKOS_ISSUER=https://api.workos.com/user_management/test-client`); its intended `https://api.workos.com` invocation passes `1/1`. | Frontend integration owner combines this bounded backend commit with the separate Playwright/bootstrap correction. Then fresh independent Security and QA gate the exact integrated SHA; do not release on the implementation evidence alone. |
| DB-035B | P2 | Implementation complete — independent gates pending | Make the new-presentation authenticated rail describe work, management, and communication rather than one undifferentiated destination list. | Frontend & UX Recovery Owner | The Utility Inbox direction in `docs/DB-035_UTILITY_INBOX_NAVIGATION_DECISION.md` is implemented: Shop Work → Repair Orders → Customers remains contiguous; My Shop is a distinct management entry; Messages is account-adjacent with the existing role gate and unread affordance. Authenticated desktop/compact/mobile verification confirms the responsive IA and compact Messages actions meet the 44px target; routes, permissions, unread query, legacy presentation, and mobile More navigation remain authoritative. | Fresh independent Security and QA on the exact candidate; no backend/API/migration or navigation-contract expansion is required. |
| DB-035C | P0 | Implementation complete — independent gates pending | Replace the new-presentation customer Sidekick primary detail with a canonical page-owned customer workstation while preserving legacy rollback. | Frontend & UX Recovery Owner | `docs/DB-035_CUSTOMER_WORKSTATION_CONTRACT.md` is implemented with additive query-preserving `?selected={customerId}` state, wide navigator + persistent workspace, compact list-to-workspace transition, Back/focus restoration, in-workspace vehicle drilldown, existing queries/mutations/permissions only, and unchanged legacy Sidekick. Flat responsive customer records support independent **Details** inspection and canonical selection; one-at-a-time disclosure does not navigate or replace the selected workspace, while the progressive **Open customer** action updates selection. Authenticated `1440/960/390/320` checks confirm no page/navigator overflow, selected-versus-inspected independence, coherent responsive hierarchy, and theme-safe presentation. No backend/API/migration/business-logic/auth change was made. | Fresh independent Security and QA on the exact candidate; no further Product implementation scope is open. |
