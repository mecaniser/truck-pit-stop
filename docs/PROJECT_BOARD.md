...
| DB-035 | P1 | Released — Truck Pit Stop all-user rollout active | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Frontend & UX Recovery Owner | PR #264 merged as `4cbaec940c8ed6bb7b7bdf04950f5a5eb3d11c89` after independent Security and QA GO with `P0/P1/P2=0` and all six protected checks. Current production main `78f918b67e1c42a3bb995f6504c2b1c1ff3d2cba` is healthy in Railway deployment `c3ea3430-dd99-42fa-82b5-4716b0a20010`. Product authorized all users of the exact active `Truck Pit Stop` tenant (`truck-pit-stop`, `828acd84-38bf-4a66-8fa1-cdf076f4e241`): production changed its tenant default from `legacy` to `new` for 8 active users after confirming zero active per-user overrides and global force-legacy unset. Readback confirmed `new`; `Truck Pit Stop Wisconsin` remains `legacy`; 77 Cargo remains the fleet-company context, not a tenant rollout target. Web and API health returned HTTP 200. | Release runs one authenticated Truck Pit Stop staff canary after reload/new session and monitors client/API errors. Rollback is the single tenant default value `legacy`; no user override, Wisconsin tenant, 77 Cargo record, route, permission, or business state changed. |
