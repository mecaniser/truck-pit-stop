...
| DB-035 | P1 | Finite completion pass | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Frontend & UX Recovery Owner | Product authorized the finite completion program after accepting the current local sequencing checkpoint. Focused local commits now isolate the shared daily workbench, utility inbox, committed Appearance lifecycle/Fleet return context, page-owned Repair Orders workspace, and page-owned Customers workstation. The current stabilization delta adds one canonical search-field contract and consistent protected master/detail row activation across Customers and Repair Orders. Focused Customers/Repair Orders tests pass `37/37`; focused workbench/shell/Appearance tests pass `53/53`; TypeScript/production build, changed lint and diff integrity pass. The canonical `5173`/`8000` runtime remains sourced from the recovery worktree. | Complete the existing Messages, My Shop and Settings structural verification, then run the fixed cross-surface matrix and fresh independent QA/Security. Authenticated browser acceptance currently requires Product to restore the expired WorkOS session in the visible tab. No DB-036, PR, merge, deployment or self-gate is authorized. |
