...
| DB-035 | P1 | In Progress — shared shell | Reshape and polish the authenticated garage cockpit so the landing promise is fulfilled by connected real workflows | Frontend & UX Recovery Owner | Accepted baseline `a7b7fc0879af2b811ccf5c04b224133e6f7e9d0c` retains the backend/Appearance foundation and bounded Harden cleanup. Product rejected Adapt `5c8c698` because a rail and queue rearrangement did not deliver the connected operational outcome despite green automation. The controlled plan is `docs/DB-035_MASTER_EXECUTION_PLAN.md`. Product selected Action Ledger plus a compact Record Atlas connected-record strip and on 2026-08-12 approved the corrected landing-derived navy-shell/road-white-workspace prototype at 1440/960/390/320. The canonical local pair remains frontend `5173` with strict-port failure and backend `8000`. | Implement Stage 2 only: shared authenticated shell behind the existing presentation flag, with legacy rollback and no Shop Cockpit/Repair Orders recomposition yet. Product reviews the shell before Stage 3. Fresh independent Security, QA and Impeccable/Emil gates follow the complete candidate. |
