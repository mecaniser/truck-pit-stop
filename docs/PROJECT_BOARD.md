...
| DB-035C | P0 | In progress — customer workstation | Replace the new-presentation customer Sidekick primary detail with a canonical page-owned customer workstation while preserving legacy rollback. | Frontend & UX Recovery Owner | Product accepted `docs/DB-035_CUSTOMER_WORKSTATION_CONTRACT.md`: additive `?selected={customerId}` state, wide navigator + persistent workspace, compact list-to-workspace transition, in-workspace vehicle drilldown, existing queries/mutations/permissions only, and unchanged legacy Sidekick. No backend/API/migration/business-logic/auth change is authorized. | Implement the bounded Customers slice, then require focused selection/history/mutation/rollback tests, authenticated 1440/960/390/320 acceptance, appearance/forced-color/contrast checks, full frontend/build/lint/diff, and final Impeccable detector. No PR, merge, or deployment is authorized. |
