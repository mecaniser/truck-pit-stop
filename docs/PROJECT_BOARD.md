...
| DB-002 | P0 | Review | Ship the supported paid repair-order conversion export | Backend & Integrations | Reconstructed on `codex/conversion-export` from `2bdc53c`; migration 108 remains single and already precedes current head. Local implementation adds public-only destination validation with redirects/proxy inheritance disabled, cross-tenant resource guards, serialized correction idempotency, amount rules, API-only contract documentation, and 38/38 focused tests. Full backend baseline: 571 passed, 31 failed, 3 skipped; failures are the pre-existing DB-018 baseline and not in DB-002 focused coverage | Independent Architecture/Security and QA gates; PostgreSQL migration rehearsal; configure production encryption secret; prove worker/beat and signed test delivery; focused PR/CI, Railway deploy and canary |
