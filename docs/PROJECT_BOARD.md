...
| DB-037 | P1 | Ready for local adoption — Security and QA GO | Provide one MVP local controller that switches fixed ports 5173/8000 between clean registered worktrees without mixed frontend/backend sources and shows `branch@shortSHA` in development. | Release & Reliability | Exact implementation `094d75232914d4146d6c88135ed115a46a9c7764` includes the bounded controller, Vite identity injection, and development-only authenticated-shell indicator. Independent Security GO and QA GO report `P0/P1/P2=0`: controller `27 passed/1 sandbox-only skip`, focused frontend `11/11`, production build and diff-check pass, unknown/mixed/dirty/migration and rollback cases fail closed, and production artifacts contain no runtime marker. Ports 5173/8000, Docker, and the local database were unchanged throughout verification. Deferred hardening remains outside v1. | Product may use the controller locally after the branch is integrated into a reachable checkout. No push, PR, merge, production deployment, or automatic runtime switch is authorized or performed. |
